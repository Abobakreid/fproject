...
const PlayerReservation = () => {
  ...
  const getMyReservations = async () => {
    try {
      ...
      if (data) {
        ...
        await filtWithOutemail.forEach(async (item) => {
          console.log(item.id, "done");
          await getstaddata(item.id);
        ...
        if (filt[0].isStadiumRated) {
          console.log("s");
        } else {
          navigate(`/addreview/${filt[0].stadiumId}?${filt[0].teamId}`, {
            replace: true,
          });
        }
        setLaod(false);
  ...
  const getstaddata = async (d) => {
    try {
      ...
      const data = await res.json();
      console.log(data);
      console.log(stad_Id, users.id, d, data, "await");
  ...
  return (
    <div className="player_reservation">
      <Navbar
        login={"register"}
                  ...
                  return (
                    <div className="p_team" key={index}>
                      <div className="up">
                        ...
                        <div className="allowRate">
                          {item.position == 3 ? (
                            <>
                              {" "}
                              {playerRatem.includes(item.id) ? null : (
                                <NavLink
                                  to={`/rateofgoalkeeper/${item.id}?${stad_Id}`}
                                >
                                  Add Rate
                                </NavLink>
                              )}
                            ...
                            <>
                              {playerRatem.includes(item.id) ? null : (
                                <NavLink
                                  to={`/rateofplayer/${item.id}?${stad_Id}`}
                                >
                                  Add Rate
                                </NavLink>
                              )}
